...
To get started make sure you have `contour-authserver` and the `ExtensionService` deployed as described in above. 
...
You can also exclude a TLS virtual host from the global external autohorization policy by adding the `globalExtAuthDisabled` flag. 
...
### Overridng global external authorization for a virtual host

In some cases you may want a different configuration that what is defined globally. To override the global external autohorization add the `authorization` block to your HTTPProxy as shown below
